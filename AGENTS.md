...
- `scene_pressure_cell_case.{h,cpp}` exposes that owner as the selectable
  ...
  prescribed periodic mean wind through a uniform pump, advances the collapsed
  MAC predictor with the projected nonlinear SSPRK2 bulk operator, and applies
  only the final scene pressure load to Structure. Immutable frames publish
  displacement, triangle pressure jump, pressure/pump vectors, bulk-advection
  change/divergence, iteration count, bulk MAC speed, and subface-collapse
  spread. The final projection is cut-region-aware, but the intermediate bulk
  projections do not retain distinct per-region subface velocities. Its
...
- `scene_pressure_cell_checkpoint_persistence.{h,cpp}` owns the bounded
  `SWPCELL3` canonical restart envelope. It persists the trusted nested
