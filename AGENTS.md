...
- `simwing_scene_fluid_pressure_face_link`: binds those pressure unknowns
  ...
  distances, exact count and area, and publishes no fabricated link.
...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  Resolving those embedded conductances and the subsequent worker path remain
  open.
