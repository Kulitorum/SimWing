...
- `simwing_scene_fluid_geometry`: builds a bounded deterministic cell-major
  ...
  clipped-polygon segments into unique oriented internal-face crossings. None
  of these products is a cut-cell volume or fluid-region classification.
...
There are 121 configured tests on Windows. The Fortran-reference test is
...
| SimWing authoritative scene/grid geometry | `simwing_scene_fluid_surface`, `simwing_scene_fluid_surface_transfer`, `simwing_scene_surface_geometry`, `simwing_scene_surface_intersection`, `simwing_scene_surface_clipping`, `simwing_scene_surface_ownership`, `simwing_scene_surface_crossings`, `simwing_scene_fluid_quadrature`; preserve complete accepted-state fingerprints, canonical bounded broad/narrow phases, exact barycentric clipping and area partition, explicit point/segment contact, duplicate shared-plane masks, unique internal face ownership with authored sides/winding, exact paired transverse face segments with stable identity and in-face authored-side direction, contact/coplanar separation, periodic-boundary and grid-edge ambiguity rejection before explicit ownership exists, stable unique-area quadrature IDs and physical metadata, ordered traction binding, and no shared-plane force double counting |
