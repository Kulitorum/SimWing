...
.\build\bin\Release\simwing-fsi.exe [--case structural|piston|open-piston|periodic-flow|pressure-jump] [--steps N] [--trace <file>] [--checkpoint-in <file>] [--checkpoint-out <file>] [--checkpoint-every N] [--control-stdio] [--no-viewer]
...
There are 81 configured tests on Windows. The Fortran-reference test is
