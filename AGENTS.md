...
- `scene_pressure_cell_case.{h,cpp}` exposes that owner as the selectable
  ...
  and iteration count through immutable viewer frames. Its in-memory
  checkpoint must replay the exact next frame. It remains a fixed-MAC-predictor
  canonical, not a wing or full CFD claim.
- `scene_pressure_cell_checkpoint_persistence.{h,cpp}` owns the bounded
  `SWPCELL1` canonical restart envelope. It persists the trusted nested
...
| SimWing scene pressure feedback | `simwing_scene_fluid_pressure_coupling`, `simwing_coupling`; preserve exact macro-step baseline rewind before every nonlinear solve, topology-stable consecutive pressure epochs, trapezoidal accepted-start/relaxed-end pressure load application, the existing conservative pressure-to-Structure path, displacement/velocity/applied-versus-solved-load convergence, deterministic Aitken continuation, accepted pressure warm-state continuation, exact next-step replay, and full Structure/pressure-owner rollback on exhaustion, topology rejection, projection failure, or exception; do not claim evolving momentum or topology rebase while the MAC predictor remains fixed |
| SimWing visible scene pressure cell | `simwing_scene_pressure_cell_case`, `simwing_fsi_pressure_cell_headless`, and the `simwing_fsi_pressure_cell_checkpoint_*` chain; preserve byte-deterministic accepted frames, three fixed intake vertices, topology-stable moving pressure epochs, bounded deterministic initial/accepted checkpoint round trips, exact split-run next-frame replay, transactional foreign/corrupt/truncated/trailing/oversized rejection, centimetre-scale bounded driven motion, per-triangle area-weighted pressure jump, nodal/total pressure and actuator vector fields, strong-iteration telemetry, and explicit fixed-MAC-predictor diagnostic scope |
