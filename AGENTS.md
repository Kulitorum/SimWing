...
- `scene_fluid_mimetic_trace_solve.{h,cpp}` is the audit-only transactional
  gauge-fixed Jacobi-PCG solve over that matrix-free system. It admits only
  bounded component-sum roundoff, uses the exact condensed diagonal, publishes
  only after a fresh full-residual check, and preserves the warm start exactly
  on incompatibility, non-finite arithmetic, or exhaustion. Manufactured
  multi-component fields recover, source-driven local/trace conservation
  closes, and one complete real-wing iteration remains finite, reduces the
  residual, and rolls back when truncated. Jacobi is not yet a practical
  full-wing preconditioner; material-wall trace condensation or stronger
  local/multilevel preconditioning, physical RHS assembly, and production
...
- `simwing_fluid`: Qt-free periodic staggered-grid field operators and
  ...
  builds it without dense local storage. It does not alter the production
...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  lie on the positive side of each local cap plane, so never hide this
   ...
   closes compatible source-driven fluxes, and executes a finite
   residual-reducing iteration across the full real system with exact rollback
   when truncated. Practical wall-trace condensation/preconditioning and
   production integration remain open. The production operator and subsequent
   worker path are unchanged.
...
| SimWing mimetic trace system/solve | `simwing_scene_fluid_mimetic_trace_system`, `simwing_model_scene_real_export`; preserve exact two-incidence Cartesian/opening trace pairing, unique one-incidence zero-flux material-wall traces, deterministic component gauges, rejection of disconnected within-component trace topology, compact local-kernel binding, positive Jacobi diagonals, matrix-free symmetric positive-semidefinite action, one component-constant null mode, compatible source-derived right-hand sides, bounded storage and nested fingerprint/corruption rejection; preserve gauge-fixed Jacobi-PCG manufacture, declared component-roundoff correction only, exact gauge publication, freshly recomputed full-residual convergence, local/source/trace closure, transactional rollback on incompatibility/non-finite arithmetic/exhaustion, and the finite residual-reducing truncated coarse 191,579-trace/13,132,336-local-byte audit without changing production pressure arithmetic |
