...
- `scene_pressure_cell_checkpoint_persistence.{h,cpp}` owns the bounded
  `SWPCELL5` canonical restart envelope. It persists the trusted nested
  ...
  Projection persistence includes optional link-continuation provenance, but
  this worker rejects a nonzero marker until it owns cut-region transport.
