...
- `scene_fluid_region_transport.{h,cpp}` owns the first region-momentum step.
  ...
  publication. It does not yet model material-wall viscosity, topology rebase,
  or perform the next pressure projection.
...
- `scene_pressure_cell_case.{h,cpp}` exposes that owner as the selectable
  ...
  velocities and bulk viscosity is not wall-conditioned. After bootstrap it
  owns accepted region momentum, applies the bulk-MAC predictor delta,
  transports that state through moving-volume GCL, and uses the resulting
  current-link predictor in every strong iterate. Its
...
- `scene_pressure_cell_checkpoint_persistence.{h,cpp}` owns the bounded
  `SWPCELL7` canonical restart envelope. It persists the trusted nested
  ...
  complete accepted region-momentum state and projection provenance are
  serialized; the derived accepted MAC field and transient bulk pressure are
  not.
