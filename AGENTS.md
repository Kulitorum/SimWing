...
- `scene_pressure_cell_mimetic_conductance_phase_refinement_audit.{h,cpp}`
  ...
  `0.100660/0.01043`, `0.240930/0.39050`, and `0.521248/0.38828`; the fine
  range is `0.293734`-`0.888671`. The former five fine-grid rejections were
  ...
  unchanged `1e-10` algebraic tolerance. This removes graph censoring from the
  shadow-only matrix but is not a convergence claim, and the area-weighted
...
- `src/fsi/fluid/scene_surface_face_graph.{h,cpp}` stitches face-local segment
  ...
  triangle plus the two exact Cartesian planes, retains authored opening
  boundaries, and publishes bounded node degrees/connectivity. It does
