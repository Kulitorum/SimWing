...
There are 97 configured tests on Windows. The Fortran-reference test is
...
| SimWing moving porous full-flow case | `simwing_moving_porous_flow_case`; preserve deterministic owning frames, the first-step `face=3,image=0` to `face=0,image=1` staged wrap, continuous accumulated fluid state, five topology rebases through the second wrap at step 101, complete final sheet/pump crossing planes, both unwrapped stage epochs, closed kinematic residual, positive porous dissipation, bounded momentum residual, and immutable earlier frames |
