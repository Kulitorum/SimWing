...
- `scene_fluid_pressure_operator_response_audit.{h,cpp}` is the offline
  ...
  the accepted source plus five deterministic component-compatible coordinate/
  high-frequency modes through independent graph and mimetic solves, aligns
  control pressures by component gauge, retains every source/response/residual
  row, and fingerprints solver residuals and fitted gain/shape diagnostics.
  On the 65-control pressure cell the accepted direction has about `2.54` gain
  with `2.4%` control-field shape residual, while the five independent modes
  have `0.999`-`1.007` gains and `1.2%`-`16.5%` residuals. The live mismatch is
  therefore mode-specific, not a global pressure-unit correction. This audit
