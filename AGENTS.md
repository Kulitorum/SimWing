...
- `scene_fluid_mimetic_control_cell.{h,cpp}` is an audit-only immutable adapter
  ...
  On the coarse real wing, 130 of 138 controls close geometrically; all eight
  Outside controls remain open because ten untouched Cartesian faces are
  ambiguous when both adjacent cells contain many authored regions. The audit
  conservatively marks incident controls incomplete and never labels a face
  from dominant volume or closure. Its 42,826 cell-owned opening patches become
  85,652 paired half-faces, so rejected two-point apertures are not missing
  geometry. The manual `4 x 4 x 4` audit has 358 controls, 346 geometrically
  closed, six ambiguous faces, 12 open Outside controls, and 250 conservatively
  ready controls. It safely omits 240 zero-volume material sides, misses no
  opening side, retains 95,984 paired opening half-faces, and reaches 2,947
  half-faces in its largest control. It does not replace the production graph
  operator.
 - `simwing_scene_fluid_pressure_face_link`: binds those pressure unknowns
  ...
  exactly one region. A face-aligned authored
...
- `simwing_fluid`: Qt-free periodic staggered-grid field operators and
  ...
  exactly to `area / centre-distance` on condensed Cartesian neighbors. It is
  not yet assembled from scene cut cells and does not alter the production
...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  lie on the positive side of each local cap plane, so never hide this
   ...
   theorem nonclosure. It closes 130 of 138 coarse real-wing controls; ten
   untouched ambiguous faces leave the eight Outside controls open. Resolving
   those face-region labels authoritatively and building the global trace
   system remain open. The `4 x 4 x 4` manual audit still has six ambiguous
   faces and 12 open Outside controls, so refinement alone is not the closure.
...
There are 159 configured tests on Windows. The Fortran-reference test is
