...
- `scene_fluid_mimetic_trace_solve.{h,cpp}` is the audit-only transactional
  ...
  full-wing preconditioner. The exact local wall Schur kernel now reduces the
  real system's 148,652 material-wall traces to 42,927 shared traces, but its
  global condensed-system assembly/solve, any stronger local/multilevel
  preconditioning, physical RHS assembly, and production integration remain
  open.
...
- `simwing_fluid`: Qt-free periodic staggered-grid field operators and
  ...
  builds it without dense local storage. Its
...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  lie on the positive side of each local cap plane, so never hide this
   ...
   when truncated. Exact local wall-trace condensation now accepts all coarse
   controls; global reduced-system assembly/preconditioning and production
