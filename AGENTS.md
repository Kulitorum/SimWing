...
- `scene_pressure_cell_operator_phase_refinement_audit.{h,cpp}` nests that
  ...
  the phase set before the paired response audit. The graph-independent
  terminal primitive now exists; applying it to every phase without first
  building the graph operator is the next required discriminator.
