...
There are 153 configured tests on Windows. The Fortran-reference test is
...
| SimWing visible scene pressure cell | `simwing_scene_pressure_cell_case`, `simwing_fsi_pressure_cell_headless`; preserve byte-deterministic accepted frames, three fixed intake vertices, topology-stable moving pressure epochs, exact checkpoint next-frame replay, centimetre-scale bounded driven motion, per-triangle area-weighted pressure jump, nodal/total pressure and actuator vector fields, strong-iteration telemetry, and explicit fixed-MAC-predictor diagnostic scope |
