...
.\build\bin\Release\simwing-fsi.exe [--case structural|piston|open-piston|periodic-flow] [--steps N] [--trace <file>] [--checkpoint-in <file>] [--checkpoint-out <file>] [--checkpoint-every N] [--control-stdio] [--no-viewer]
...
- `src/fsi/open_piston_case.{h,cpp}` is the third worker case selected with
  ...
  and preserves the caller's prior output on any failed decode. The worker CLI
...
There are 77 configured tests on Windows. The Fortran-reference test is
