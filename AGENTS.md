...
- `simwing_scene_fluid_geometry`: builds a bounded deterministic cell-major
  ...
  grid. Its padded triangle AABBs are candidates, not cut-cell classifications.
...
- `src/fsi/fluid/scene_surface_geometry.{h,cpp}` conservatively bins every
  ...
  selection, exact triangle/cell intersections, face crossings, volume
  fractions, and region reconstruction remain explicit future stages.
...
There are 116 configured tests on Windows. The Fortran-reference test is
