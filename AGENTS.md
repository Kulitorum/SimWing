...
- `scene_pressure_cell_case.{h,cpp}` exposes that owner as the selectable
  ...
  publishes displacement, triangle pressure jump, pressure/actuator vectors,
  iteration count, bulk MAC speed, and subface-collapse spread through
  immutable viewer frames. It advances the pressure-corrected collapsed MAC
  predictor between accepted steps. Its in-memory checkpoint must reconstruct
  that derived predictor and replay the exact next frame. It remains a reduced
  pressure-feedback canonical, not a wing or full CFD claim.
- `scene_pressure_cell_checkpoint_persistence.{h,cpp}` owns the bounded
  `SWPCELL2` canonical restart envelope. It persists the trusted nested
  ...
  validates exact next-frame replay before decoded state becomes visible.
...
| SimWing visible scene pressure cell | `simwing_scene_pressure_cell_case`, `simwing_fsi_pressure_cell_headless`, and the `simwing_fsi_pressure_cell_checkpoint_*` chain; preserve byte-deterministic accepted frames, three fixed intake vertices, topology-stable moving pressure epochs, pressure-corrected bulk MAC continuation between accepted steps, reported maximum MAC speed/subface collapse spread, exact derived-predictor reconstruction after restore, bounded deterministic initial/accepted checkpoint round trips, exact split-run next-frame replay, transactional foreign/corrupt/truncated/trailing/oversized rejection, centimetre-scale bounded driven motion, per-triangle area-weighted pressure jump, nodal/total pressure and actuator vector fields, strong-iteration telemetry, and explicit reduced-pressure-feedback diagnostic scope |
