...
- `scene_fluid_mimetic_trace_solve.{h,cpp}` is the audit-only transactional
  ...
  full-wing preconditioner. The exact local wall Schur kernel and immutable
  global condensed adapter now reduce the real system's 148,652 material-wall
  traces to a separate 42,927-row shared system. Its reduced solve, any stronger
...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  lie on the positive side of each local cap plane, so never hide this
   ...
   controls and its global 42,927-row shared system is assembled with full wall
   reconstruction. Reduced solving/preconditioning and production integration
   remain open. The production operator and subsequent worker path are
   unchanged.
