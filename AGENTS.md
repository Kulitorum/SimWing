...
- `simwing_scene_fluid_cell_volume`: reconstructs bounded deterministic sparse
  per-cell region volumes for the first closed-manifold subset. It requires one
  Outside region, no openings, consistently wound two-sided surfaces, and a
  resolved partition on every active MAC face; per-cell closure must also agree
  with independent whole-surface region volumes. General open-intake and
  unresolved/coplanar/grid-edge cases remain unsupported.
...
- `src/fsi/scene_fluid_cell_volume.{h,cpp}` consumes one validated grid epoch
  and applies the divergence theorem to cell-owned material patches plus
  resolved per-region MAC-face areas. It publishes positive sparse region
  volumes, enforces every cell's full volume, and cross-checks their global sum
  against a separate closed-surface calculation. It rejects openings,
  non-manifold/inconsistently wound surfaces, coplanar owners, unpaired
  crossings, and unresolved active faces rather than guessing fluid regions.
...
There are 132 configured tests on Windows. The Fortran-reference test is
...
| SimWing closed-manifold cell volumes | `simwing_scene_fluid_cell_volume` plus the authoritative scene/grid geometry checks when their inputs change; preserve epoch binding, deterministic sparse ordering, exact analytic nested volumes, local/global closure, bounded publication, and explicit rejection of openings or unresolved topology |
