...
- `scene_fluid_region_wall.{h,cpp}` remaps one immutable accepted transport to
  each current strong-coupling geometry and applies two-sided tangential
  ...
  is a coarse topology-stable wall closure, not a resolved boundary layer.
- `scene_fluid_region_link_flow.{h,cpp}` remaps an accepted fixed-epoch
  ...
  provenance. Topology changes remain outside the contract.
- `scene_pressure_cell_case.{h,cpp}` exposes that owner as the selectable
  ...
  spread. The final projection is cut-region-aware, but the intermediate bulk
...
- `scene_pressure_cell_checkpoint_persistence.{h,cpp}` owns the bounded
  `SWPCELL8` canonical restart envelope. It persists the trusted nested
  ...
  and projection provenance are serialized; the derived accepted MAC field and
  transient bulk pressure are not.
