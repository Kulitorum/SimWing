...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  lie on the positive side of each local cap plane, so never hide this
   ...
   Cartesian equivalence on manufactured closed cells. Scene integration still
   requires one immutable complete unwrapped half-face shell per sparse
   cell/region, combining Cartesian region subfaces, material-wall quadrature,
   and opening-cap patches and rejecting area-vector or divergence-theorem
   nonclosure. The production operator and subsequent worker path remain open.
