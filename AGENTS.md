...
- `scene_fluid_pressure_projection.{h,cpp}` supplies the first physical
  fixed-epoch adapter on that topology. It maps predicted MAC normals to one
  ...
  continuity check. It does not reconstruct one ambiguous corrected MAC value
  across partitioned faces and does not yet add moving-volume/GCL sources or
  off-face opening transmissibility.
...
There are 147 configured tests on Windows. The Fortran-reference test is
