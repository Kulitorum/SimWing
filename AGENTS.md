...
.\build\bin\Release\simwing-fsi.exe [--case structural|hemisphere|flag|ram-cell|piston|strong-piston|open-piston|periodic-flow|porous-flow|moving-porous-flow|porous-sheet|pressure-jump] [--steps N] [--trace <file>] [--checkpoint-in <file>] [--checkpoint-out <file>] [--checkpoint-every N] [--control-stdio] [--no-viewer]
...
- `simwing_scene_fluid_opening_cap`: builds topology-only virtual caps for the
  first planar strictly convex authored-opening subset. Every separating
  surface boundary edge must be claimed exactly once; adjacent fabric fixes
  negative-to-positive winding. Caps follow accepted Structure vertices but
  never become fabric or traction. Its companion one-point triangle
  ...
  once per cell or non-periodic face and retain their clipped polygons. A
...
- `simwing_scene_fluid_cell_volume`: reconstructs bounded deterministic sparse
  ...
  interior cells and open face-local tile chains. Each tetrahedron and cell
  closes independently before the result agrees with whole-surface region
  volumes. Projection-connected opening flux, nonplanar/concave mouths,
...
- `simwing_scene_fluid_pressure_control_volume`: turns every positive sparse
  ...
  preserves cell/region/component ownership, derives a stable ID from the
  fixed-grid cell/region key, and selects a deterministic gauge control volume
  in each authored-opening component.
- `simwing_scene_fluid_pressure_face_link`: binds those pressure unknowns
  ...
  link per positive region area; provably untouched faces link only when their
  adjacent sparse cells share exactly one region. A face-aligned authored
  opening creates oriented cross-region links over its exact patch area and an
  unambiguous same-region link over the complementary face area. Off-face
  openings, material/open-chain/coplanar overlap, and ambiguous inactive faces
  remain explicit and unlinked. General opening coupling and projection remain
...
- `simwing_scene_fluid_pressure_operator`: assembles those links into a
  ...
  link-connected graph per authored component. Resolved face-aligned openings
  join their regions; unresolved and off-face openings reject instead of
  becoming implicit sealed walls. Its deterministic component-wise CG solve
...
- `scene_fluid_pressure_projection.{h,cpp}` supplies the first physical
  ...
  opening flux on face-aligned aperture links, assembles `-rho/dt` times net
  ...
  does not support topology rebases or off-face opening transmissibility.
- `scene_fluid_pressure_volume_rate.{h,cpp}` matches two consecutive accepted
  ...
  per-control-volume, component, and global geometry-volume rates. It requires
  identical positive sparse topology. Cell-crossing appearance/disappearance
  rejects until a conservative pressure-state remap owner exists.
...
- `scene_fluid_pressure_epoch.{h,cpp}` atomically composes one accepted grid
  remap, opening cap/patch topology, sparse cell volumes, pressure controls,
  face links, and ungauged operator. Its fingerprint binds the complete chain
  to one Structure surface state and its aggregate payload is bounded. It owns
  geometry/operator preparation only, not velocity, a pressure solution, or
  Structure mutation.
- `scene_fluid_pressure_coupling.{h,cpp}` owns the first topology-stable strong
  ...
  sweep, with the lost mixed-subface velocity spread reported explicitly. This
  is a continuation primitive; it does not yet own cut-cell advection,
  viscosity, or topology rebase/remap.
...
- `scene_fluid_region_momentum.{h,cpp}` reconstructs accepted corrected
  ...
  pressure volume. Incident normals are area-averaged per Cartesian component;
  missing components use the cell-centred value from the exact MAC predictor
  fingerprinted by the projection. Its momentum/energy/fallback and
  reconstruction-residual ledgers are the input boundary for future
  conservative region transport, not a time advance or wall model.
- `scene_fluid_region_transport.{h,cpp}` owns the first fixed-epoch region
  momentum step. Corrected relative link flow carries donor-cell vector
  momentum; graph viscosity applies equal-and-opposite impulses. Bounded
  deterministic subcycling enforces local outgoing-volume and viscous row
  limits, while stage energy and global momentum ledgers gate publication.
  It explicitly rejects moving-volume projections and does not yet model
  material-wall viscosity or perform the next pressure projection.
- `scene_fluid_region_link_flow.{h,cpp}` remaps an accepted fixed-epoch
  ...
  averages endpoint velocity onto each current Cartesian link, and subtracts
  ...
  volume-rate product; topology changes and material-wall flux remain outside
  the contract.
- `scene_pressure_cell_case.{h,cpp}` exposes that owner as the selectable
  ...
  displacement, triangle pressure jump, pressure/pump vectors, bulk-flow
  change/divergence/viscous loss, iteration count, bulk MAC speed, and
  subface-collapse spread. The final projection is cut-region-aware, but the
  intermediate bulk projections do not retain distinct per-region subface
  velocities and bulk viscosity is not wall-conditioned. Its
...
- `scene_pressure_cell_checkpoint_persistence.{h,cpp}` owns the bounded
  `SWPCELL6` canonical restart envelope. It persists the trusted nested
  ...
  derived accepted MAC field and transient bulk pressure are not serialized.
  Projection persistence includes optional static-link and transported-region
  predictor provenance, but this worker rejects either nonzero marker until it
  owns the accepted region-momentum state.
...
- `src/fsi/scene_fluid_surface.{h,cpp}` extracts stable-ID-sorted compact fluid
  regions, used porous materials, oriented two-sided triangles, and ordered
  openings directly from validated scene-v2, then captures one immutable
...
- `src/fsi/fluid/scene_surface_clipping.{h,cpp}` clips each exact pair against
  ...
  and coincident-boundary masks. A triangle lying on a shared grid plane stays
...
- `src/fsi/fluid/scene_surface_crossings.{h,cpp}` extracts positive-length
  ...
  clipped adjacent-cell copies only within a fixed machine-roundoff envelope
  into one stable lower-cell crossing, and carries the in-face
...
- `src/fsi/fluid/scene_surface_face_graph.{h,cpp}` stitches face-local segment
  ...
  intersections canonically, retains authored opening boundaries and explicit
  grid-edge endpoints, and publishes bounded node degrees/connectivity. It does
...
- `src/fsi/scene_fluid_opening_cap.{h,cpp}` matches every boundary edge of the
  separating surface to exactly one authored opening, derives cap winding from
  the adjacent triangle's region order, and fan-triangulates only planar
  strictly convex accepted loops. Its immutable bounded cap epoch is fluid
  topology for volume and future flux; it never enters Structure, material
  quadrature, or traction.
- `src/fsi/scene_fluid_opening_quadrature.{h,cpp}` gives every virtual cap
  ...
  ledgers exactly close rigid material-plus-cap surface sweep. This is opening
  kinematics for a future grid-resolved flux operator, not fluid sampling.
...
There are 157 configured tests on Windows. The Fortran-reference test is
...
| SimWing capped cell volumes/opening kinematics | `simwing_scene_fluid_opening_cap`, `simwing_scene_fluid_opening_quadrature`, `simwing_scene_fluid_opening_patch`, `simwing_scene_fluid_opening_flux`, `simwing_scene_fluid_cell_volume`, `simwing_scene_fluid_region_continuity`, `simwing_scene_fluid_region_connectivity`, `simwing_scene_fluid_pressure_control_volume`, `simwing_scene_fluid_pressure_face_link`, `simwing_scene_fluid_pressure_operator`, `simwing_scene_fluid_pressure_epoch`, `simwing_scene_fluid_pressure_solve`, `simwing_scene_fluid_pressure_projection`, `simwing_scene_fluid_pressure_volume_rate`, `simwing_scene_fluid_pressure_sampling` plus the authoritative scene/grid geometry checks when their inputs change; preserve accepted-state/epoch binding, atomic full pressure-geometry/operator composition with nested validation and aggregate storage limits, boundary-edge-complete topology-only cap winding, topology-stable one-point triangle kinematics, exact piecewise-linear normal surface sweep and rigid material-plus-cap GCL closure, exact polygon partition with unique cell/non-periodic-face ownership across accepted motion, complete MAC-field binding, exact partial-face flow, deterministic off-face staggered quadrature, negative-to-positive sign, moving-cap subtraction and co-moving zero-relative-flow invariance, equal-and-opposite per-region intake/crossport sources with exact global cancellation, consecutive-epoch identity and endpoint-trapezoidal `delta volume + outward relative flow` closure, explicit local mismatch reporting even when the global ledger cancels, deterministic intake/crossport connected components with one stable gauge each, exact component source cancellation, rejection of changing sealed pressure components, cell-major positive-volume pressure unknowns with stable fixed-grid cell/region IDs, exact cell/region/domain volume closure, component membership and deterministic gauge-volume ownership, exact full-face and nested mixed-region pressure-link area closure, same-region/component endpoint binding, oriented face-aligned cross-region aperture links with exact same-region complementary area, no dominant-cell/interface smearing, symmetric positive-semidefinite integrated graph energy, exact constant nullspace, component row-sum conservation, one connected link graph and retained gauge per authored component, exact aperture graph energy, deterministic manufactured multi-component and open-intake CG recovery, explicit component compatibility with roundoff-only correction, exact gauge normalization, explicitly recomputed final residual, fixed-epoch `-rho/dt` physical RHS assembly, exact MAC-versus-relative-aperture predicted flow ownership, independent per-link pressure correction, corrected control-volume continuity, rejected-attempt non-publication, stable consecutive pressure-volume identity, exact per-control/component geometry `dV/dt`, topology-rebase rejection, local `dV/dt + net flow` closure, zero-velocity pressure-driven intake inflation, exact side-cell pressure sampling, shared-component gauge safety, gauge-shift-invariant traction, conservative pressure load delivery, independent-gauge rejection, transactional incompatibility/non-convergence rollback, rejection of unresolved and off-face opening connectivity, periodic-boundary ambiguity rejection, no cap-to-Structure/traction path, deterministic signed-tetrahedron clipping and sparse ordering, exact analytic nested/open/large-cavity volumes including full interior cells, local/global closure, bounded work/publication, and explicit rejection of nonplanar/concave openings or non-manifold/inconsistent winding |
| SimWing scene pressure feedback | `simwing_scene_fluid_pressure_coupling`, `simwing_coupling`; preserve exact macro-step baseline rewind before every nonlinear solve, topology-stable consecutive pressure epochs, trapezoidal accepted-start/relaxed-end pressure load application, the existing conservative pressure-to-Structure path, displacement/velocity/applied-versus-solved-load convergence, deterministic Aitken continuation, accepted pressure warm-state continuation, exact next-step replay, and full Structure/pressure-owner rollback on exhaustion, topology rejection, projection failure, or exception; accepted corrected-flow MAC collapse must restore oriented opening-cap sweep, close every face's total volume flow, quantify mixed-subface velocity loss, and remain a continuation primitive rather than a claim of cut-cell advection, viscosity, or topology rebase; the explicit inverse link-flow continuation must bind consecutive topology and velocity/opening identities, carry absolute subface deviations, current-area-recentre them, reapply current cap sweep, close the bulk face flow, reject corruption/limits, and remain disabled in the worker while the static-carry negative oracle proves that missing region-resolved transport drains pressure load; reconstructed region momentum must bind the accepted pressure/volume/link/opening/predictor chain, area-average incident absolute normals by component, use only the exact pressure-predictor MAC value for unsampled components, preserve deterministic momentum/energy/fallback ledgers, and make no transport or wall-model claim |
| SimWing scene region momentum | `simwing_scene_fluid_pressure_projection`; preserve immutable region-momentum reconstruction identity and exact MAC fallback ownership, fixed-epoch donor-cell vector-momentum transfer over corrected relative links, equal-and-opposite graph-viscous impulse, deterministic local outgoing-Courant/viscous-number subcycling, non-increasing stage energy, global momentum conservation, bounded transactional non-publication, exact zero fixed points, topology-stable first-order GCL remap at retained region velocity, current-volume momentum accounting, endpoint-averaged current-link prediction, exact authored-opening cap-sweep subtraction, explicit projection provenance, and rejection of topology rebase, material-wall viscosity, or accepted-worker-state claims |
| SimWing visible scene pressure cell | `simwing_scene_pressure_cell_case`, `simwing_fsi_pressure_cell_headless`, and the `simwing_fsi_pressure_cell_checkpoint_*` chain; preserve byte-deterministic accepted frames, three fixed intake vertices with no mechanical actuator, the prescribed periodic mean-wind pump, private-candidate symmetric SSPRK2 viscosity/projected-nonlinear-advection/viscosity bulk prediction with finite acceptance, bounded final divergence, nonnegative viscous loss, and no predictor mutation before scene acceptance, topology-stable moving pressure epochs, final cut-region-aware pressure feedback, pressure-corrected bulk MAC continuation between accepted steps, reported maximum MAC speed/subface collapse spread, exact derived-predictor reconstruction after restore without persisting transient bulk pressure, bounded deterministic initial/accepted `SWPCELL6` checkpoint round trips with zero static-link and transported-region predictor provenance, exact split-run next-frame replay, transactional foreign/corrupt/truncated/trailing/oversized rejection, sustained millimetre-scale bounded pressure-driven motion, per-triangle area-weighted pressure jump, nodal/total pressure and global pump vector fields, bulk-flow telemetry, strong-iteration telemetry, and explicit airflow-bootstrap diagnostic scope without a general immersed-boundary advection, wall-viscosity, or aerodynamic-truth claim |
