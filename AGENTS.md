...
- `simwing_scene_fluid_opening_cap`: builds topology-only virtual caps for the
  planar simple authored-opening subset. Every separating surface boundary
  edge must be claimed exactly once; adjacent fabric fixes negative-to-positive
  winding. Convex loops retain their exact fan while concave loops use bounded
  deterministic reference-geometry ear clipping, keeping triangle identity
  stable under accepted motion. Folded, self-intersecting, degenerate, and
  nonplanar caps reject. Caps follow accepted Structure vertices but never
...
- `simwing_scene_fluid_cell_volume`: reconstructs bounded deterministic sparse
  ...
  with whole-surface region volumes. General swept-volume remap, nonplanar or
  self-intersecting mouths, surface junctions, and periodic-boundary ambiguity
  remain unsupported.
...
- `scene_pressure_cell_checkpoint_persistence.{h,cpp}` owns the bounded
  `SWPCELL9` canonical restart envelope. It persists the trusted nested
...
- `src/fsi/scene_fluid_opening_cap.{h,cpp}` matches every boundary edge of the
  ...
  the adjacent triangle's region order, preserves the exact fan for convex
  planar loops, and deterministically ear-clips concave planar reference
  geometry with bounded intersection/point tests. Accepted motion retains that
  triangle identity and rejects folds, intersections, and degeneracy. Its
  immutable bounded cap epoch is fluid
