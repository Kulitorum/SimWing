...
.\build\bin\Release\simwing-fsi.exe [--case structural|piston|open-piston|periodic-flow|porous-flow|pressure-jump] [--steps N] [--trace <file>] [--checkpoint-in <file>] [--checkpoint-out <file>] [--checkpoint-every N] [--control-stdio] [--no-viewer]
...
- `simwing_fluid`: Qt-free periodic staggered-grid field operators and
  ...
  per-tile volume-flow and dissipation ledgers, plus
...
- `simwing_porous_flow_case`: Qt-free pressure-driven porous plug worker. An
  ...
  endpoint pressure circuit enters projection and immutable layered frames.
  It remains a uniform one-degree-of-freedom verification oracle.
...
- `src/fsi/fluid/porous_interface.{h,cpp}` applies a calibrated normal
  ...
  volume flux, and nonnegative dissipation. This is an explicit flux-driven
  constitutive adapter, not yet an implicit pressure/flux coupling solve.
...
- `src/fsi/porous_flow_case.{h,cpp}` maps that accepted plug velocity to a
  ...
  verifies the endpoint through sharp-jump projection, and publishes an owning
  trace frame. The physical driving rise stays explicit in the plug ledger. It
  is not general nonuniform or moving porous coupling.
