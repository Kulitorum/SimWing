...
- `simwing_scene_fluid_geometry`: builds a bounded deterministic cell-major
  ...
  grid, then applies an exact normalized-SAT triangle/cell narrow phase. Neither
  candidate nor intersection pairs are cut-cell volume classifications.
...
There are 117 configured tests on Windows. The Fortran-reference test is
