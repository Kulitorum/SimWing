...
- `scene_fluid_mimetic_pressure_audit.{h,cpp}` owns the complete immutable
  ...
  frame/checkpoint bytes remain identical, and audit mode rejects checkpoint
  input/output until its endpoint state is composed into `SWPCELL` restart.
...
There are 163 configured tests on Windows. The Fortran-reference test is
...
| SimWing mimetic pressure shadow | `simwing_scene_pressure_cell_case`, `simwing_model_scene_real_export`, and `simwing_fsi_pressure_cell_mimetic_audit_headless`; require a complete fingerprinted endpoint over rebuilt control/full/condensed topology, predictor, physical source, nested solve diagnostics, accepted pressure state, and material samples; fixed pre-operator real-wing capture must cover all 138 controls and 42,927 shared traces without requiring the rejected graph operator; live audit must run only after graph convergence, bootstrap from fixed MAC flow, then use transported wall prediction plus consecutive warm remap, retain but never apply sampled loads, leave default frames and graph checkpoints byte-identical, rollback graph/Structure on endpoint-limit failure, and reject audit CLI checkpoint flags until composite restart is implemented |
