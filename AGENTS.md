...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  lie on the positive side of each local cap plane, so never hide this
   ...
   reconstruction. Stronger preconditioning and production integration remain
   open. The production operator and subsequent worker path are unchanged.
