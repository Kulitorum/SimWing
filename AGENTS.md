...
- `simwing_scene_fluid_cell_volume`: reconstructs bounded deterministic sparse
  ...
  Outside region, no openings, and consistently wound two-sided surfaces. Each
  oriented triangle becomes a signed tetrahedron against the grid origin;
  ...
  volumes. General open-intake and periodic-boundary ambiguity remain
  unsupported.
...
- `src/fsi/scene_fluid_cell_volume.{h,cpp}` consumes one validated grid epoch
  ...
  It rejects openings and non-manifold/inconsistently wound surfaces rather
  than guessing fluid closure.
...
There are 136 configured tests on Windows. The Fortran-reference test is
...
| SimWing closed-manifold cell volumes | `simwing_scene_fluid_cell_volume` plus the authoritative scene/grid geometry checks when their inputs change; preserve epoch binding, deterministic signed-tetrahedron clipping and sparse ordering, exact analytic nested/large-cavity volumes including full interior cells, local/global closure, bounded work/publication, and explicit rejection of openings or non-manifold/inconsistent winding |
