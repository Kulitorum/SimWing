...
- `simwing_scene_fluid_pressure_face_link`: binds those pressure unknowns
  ...
  adjacent sparse cells share exactly one region. Authored-opening faces,
  material/open-chain/coplanar faces, and ambiguous inactive faces remain
  explicit and unlinked. Opening coupling and projection remain separate
  future stages.
- `simwing_scene_fluid_pressure_operator`: assembles those links into a
  ...
  link-connected graph per authored component, so unresolved or off-face
  openings reject instead of becoming implicit sealed walls. It applies only
