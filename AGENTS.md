...
- `scene_fluid_mimetic_trace_solve.{h,cpp}` is the audit-only transactional
  ...
  local/multilevel preconditioning, accepted warm-state lifecycle, and
...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  lie on the positive side of each local cap plane, so never hide this
   ...
   accepted GCL volume rates. Stronger preconditioning, accepted warm-state
   lifecycle, and production integration remain open. The production operator
   and subsequent worker path are unchanged.
