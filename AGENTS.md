...
- `simwing_scene_fluid_opening_cap`: builds topology-only virtual caps for the
  ...
  never become fabric or traction.
...
There are 137 configured tests on Windows. The Fortran-reference test is
...
| SimWing capped cell volumes | `simwing_scene_fluid_opening_cap`, `simwing_scene_fluid_cell_volume` plus the authoritative scene/grid geometry checks when their inputs change; preserve accepted-state/epoch binding, boundary-edge-complete topology-only cap winding, no cap-to-Structure/traction path, deterministic signed-tetrahedron clipping and sparse ordering, exact analytic nested/open/large-cavity volumes including full interior cells, local/global closure, bounded work/publication, and explicit rejection of nonplanar/concave openings or non-manifold/inconsistent winding |
