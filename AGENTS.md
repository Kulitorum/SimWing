...
- `src/fsi/scene_fluid_surface.{h,cpp}` extracts stable-ID-sorted compact fluid
  ...
  accepted Structure epoch in that surface order. Opening-only construction
  vertices are rejected until they have trusted structural motion; grid
...
There are 114 configured tests on Windows. The Fortran-reference test is
