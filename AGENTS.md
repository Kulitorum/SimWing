...
- `scene_fluid_pressure_operator_response_audit.{h,cpp}` is the offline
  ...
  mismatch is therefore opening/interface conductance, not a global pressure-
  unit correction or bulk mode. This audit is never run inside live coupling
  and never applies loads.
