...
- `scene_fluid_mimetic_pressure_solve.{h,cpp}` is the audit-only atomic
  ...
  and `5.05e-10` maximum cell-conservation residual. It accepts already
  integrated cell sources; deriving them from a physical predictor and moving
  GCL remains outside this boundary.
...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  lie on the positive side of each local cap plane, so never hide this
   ...
   closure. Stronger preconditioning, predictor/GCL-to-source assembly, and
