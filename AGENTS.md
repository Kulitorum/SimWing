...
- `simwing_amr_external_flow`: opt-in AMReX 26.06 production-grid evaluation.
  Its two-level CPU boundary canonical owns face-centred storage and explicit
  positive-Y wind-tunnel physical ghosts. `simwing_amr_static_wing_interface`
  validates an authoritative scene-v2 surface, exactly clips its material
  triangles onto both Cartesian levels, and builds immutable positive-area
  cell-normal tensors plus diagnostic render geometry. The separately enabled
  `simwing_amr_external_flow_projection` canonical synchronizes coarse/fine
  face fluxes and uses AMReX composite multigrid to project a nonuniform wake
  with a `+Y` pressure reference. `simwing_amr_external_flow_case` publishes
  the averaged-down coarse pressure/velocity/divergence and refinement
  footprint as immutable CFD slices while a bounded passive marker moves in
  positive Y. Its persistent state fills physical and conservative face-linear
  fine/coarse ghosts, advances all staggered velocity components with a bounded
  donor-cell predictor, reapplies the normal-velocity boundary contract, and
  performs a fresh composite projection every step. Supplying `--scene`
  alternates bounded over-relaxed static cut-cell normal forcing with composite
  projections, accumulates same-step pressure corrections, and retains actual
  scene triangles in the frame. This is a one-way regularized diagnostic, not
  a sharp two-sided pressure-jump, viscosity/turbulence, leakage/intake,
  aerodynamic-load, or coupling claim. All AMR targets are excluded from
  ordinary builds.
  Configure with `-DSIMWING_ENABLE_AMREX_BACKEND=ON` and add
  `-DSIMWING_AMREX_ENABLE_LINEAR_SOLVERS=ON` for projection; supply an installed
  dependency through `-DSIMWING_AMREX_ROOT=<prefix>` or allow the pinned
  FetchContent fallback.
- `simwing_structure`: Qt-free SimWing-facing adapter around the retained XPBD
  primitives, with a bounded persistent composite checkpoint codec and a
  transactional zero-load rest-state audit. It links `softwing_core` and no
  Playground library.
...
- `simwing_frozen_scene_pressure_case`: input-driven whole-scene integration
  ...
  ramp the conservative fluid load delivered to XPBD over `0..3600 s`, tighten
  ...
  `--hold-preflow-load-preview` is the explicit visual bridge to that gate: it
  advances XPBD under the immutable final preflow transfer, publishes moving
  Structure frames under a distinct solver identity, and never rebuilds CFD or
  increments the moving-fluid geometry epoch. It keeps line certification but
  uses six deterministic cable sweep pairs and `60 s^-1` damping for a
  quasi-static display. Do not report it as coupled FSI.
...
- `simwing_scene_fluid_opening_cap`: builds topology-only virtual caps for
  ...
  Reference-planar convex loops retain their exact fan and reference-planar
  concave loops use bounded deterministic reference-geometry ear clipping;
  that disk and its faceted normals survive accepted nonplanar motion.
  Scene-v2.2 must instead provide one oriented boundary-vertex disk when the
  reference loop is already nonplanar. Folded, self-intersecting, degenerate,
  or unauthored reference-nonplanar caps reject.
...
- `scene_fluid_mimetic_trace_system.{h,cpp}` is the audit-only immutable
  ...
  side. The coarse real wing has 191,703 trace unknowns and 13,139,280 bytes of
...
- `scene_fluid_mimetic_trace_solve.{h,cpp}` is the audit-only transactional
  ...
  the 191,703-row full system and its 42,927-row condensed system remains
...
- `scene_fluid_mimetic_condensed_trace_system.{h,cpp}` composes the local wall
  ...
  3,988,710 bytes of local Schur data.
...
- `scene_fluid_mimetic_pressure_solve.{h,cpp}` is the audit-only atomic
  ...
  state. A balanced coarse real-wing source pair converges in 302 iterations,
  with `5.39e-9` reconstructed RMS, `2.33e-6` maximum original-row residual,
  and `2.49e-10` maximum cell-conservation residual. Results retain full,
...
- `simwing_fluid`: Qt-free periodic staggered-grid field operators and
  ...
  it, reducing 148,776 wall traces to 42,927 shared traces with 3,988,710 bytes
...
- `simwing_viewer_geometry`: Qt-free deterministic diagnostic-vector glyph and
  structured CFD-slice builders. Explicit global metadata binds an X-fast
  cell grid to unreferenced diagnostic vertices; the slice validates that
  untrusted descriptor before emitting source-indexed cell quads. The target
  links only the viewer protocol; Qt/OpenGL only uploads the resulting geometry.
...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  it now detects the two reference-collapsed gnuC2 wingtip loops, refines their
  unequal 20/15-vertex boundary paths to the union of 33 authoritative samples,
  locally splits only incident boundary triangles, and authors two exact paired
  seam chains with shared welded endpoints. Explicit seam properties are
  mandatory when such a closure exists. The cap owner now consumes the
  ...
  lie on the positive side of each local cap plane, so never hide this
   ...
   The coarse real-wing system has 191,703 trace unknowns and 13,139,280 bytes
   ...
   `1e-5` relative RMS within 300 iterations, reconstructing all 191,703 rows
...
- `src/fsi/scene_fluid_opening_cap.{h,cpp}` matches every boundary edge of the
  ...
  from the adjacent triangle's region order. Reference-planar loops preserve
  the exact convex fan or receive bounded deterministic concave ear clipping;
  the resulting facets may become nonplanar under accepted motion. A
  reference-nonplanar boundary must author its disk. Accepted motion retains
  triangle identity and individual faceted normals and rejects folds,
...
- `src/viewer/cfd_slice.{h,cpp}` validates the explicit X-fast grid descriptor
  carried by global frame fields and emits source-indexed cell quads for one
  X/Y/Z-normal plane without depending on Qt or solver storage.
- `src/viewer/viewer_window.{h,cpp}` and `tools/simwing_viewer_main.cpp` form
  ...
  A second toolbar controls the structured CFD slice, its X/Y/Z normal, and
  cell-index plane slider; active CFD vectors are filtered to that plane,
...
- Batch `simwing-fsi` launches the follow-viewer after creating the trace file
  but before constructing the selected case, so expensive real-scene setup is
  visible as an initialization state. `--trace-every N` still advances every
  solver step and writes the first, every Nth, and final accepted frame; its
  default of one preserves consecutive traces and control-stdio rejects it.
...
- `src/fsi/frozen_scene_pressure_case.{h,cpp}` is the first input-driven
  ...
  reference-collapsed material boundary edge opened by that motion. Paired
  wingtip seams now close that structural gate. On the refined surface the
  same two-epoch release instead retains a `0.571382 N` resultant, moves
  `180.471 mm` with a `1.70 um` line residual, and then rejects a reversed
  inferred cap facet at opening 278. The corrected-flow pre-roll must be
...
- `src/fsi/fluid/planar_region_fragment_opening_momentum_transport.{h,cpp}`
  ...
  prior accepted transport controls or an identity-preserving adjusted state
  directly and uses the pressure-corrected current velocity state only for
  target relative flows. The legacy state/transport overloads retain their
  arithmetic and fingerprints. Corrected same-region and aperture-relative
  flow carries the complete vector momentum by
...
| SimWing AMR external flow | opt-in `simwing_amr_external_flow`, `simwing_amr_static_wing_interface`, `simwing_amr_external_flow_projection`, and `simwing_amr_external_flow_case`; preserve the pinned AMReX version, default-build independence, two-level non-periodic block layout, the gnuC2-following half-open refinement box and 70,776 active composite cells, face-centred X/Y/Z storage, explicit `-Y` inflow / `+Y` outlet / X-Z far-field ownership, nonuniform divergent wake, exact physical-ghost checks, finite byte accounting, conservative face-linear fine/coarse transport ghost fill with composite projection owning divergence closure, coarse/fine face-flux synchronization before and after projection, homogeneous pressure-correction Neumann ownership at prescribed-normal-velocity boundaries, the `+Y` homogeneous Dirichlet pressure reference, strong composite divergence contraction, exact inlet-normal preservation, nonzero outlet adjustment, owning X-fast coarse diagnostic copies with covered values averaged down, 24,576-cell slice metadata, bounded donor-cell passive-marker transport at CFL below one, bounded donor-cell staggered momentum prediction on both levels, physical normal-velocity re-enforcement, transactional face-state rollback, deterministic immutable frames and positive-Y motion; with `--scene`, also preserve authoritative stable-ID/winding/side/role surface retention, explicit out-of-domain rejection, exact positive-area coarse/fine cell clipping, symmetric two-cell activation for face-coincident material, area-normal tensors, the twelve-pass relaxation-1.8 forcing/projection sequence, accumulated same-step pressure, whole-wing triangle rendering, real gnuC2 70,494-triangle/94.1942-square-metre/3,600-cut-cell binding, continuous CFL-bounded 20-step acceptance, and the explicit one-way regularized/no-sharp-jump/no-viscosity/no-turbulence/no-leakage/no-load/no-coupling claim. The explicit `--external-slab-x X` exception uses a fixed 1.125 m X domain, clips rather than rejects out-of-domain source material, keeps stable source triangle identity without periodic images or synthetic closing faces, and is only a rapid local interface/viewer probe; preserve resolution scales `1..4`, proportional coarse/refined-box subdivision, the scale-four `12 x 192 x 64` coarse grid and `0.0003125 s` step, and rejection of the scale option without a slab. Nearby X far-field boundaries exclude every slab scale from pressure/load/spanwise validation. AMR blocks are averaged down for publication; pressure multigrid levels are internal and are not viewer geometry. OpenMP remains opt-in for compatible toolchains and must fail configuration explicitly under unsupported MSVC rather than silently run serial. |
...
velocity state; a re-entrant epoch fully validates and directly consumes either
the prior accepted transport controls or an identity-preserving adjusted state
bound to their exact target metric. Never reconstruct prior transported or
adjusted momentum through a face-based velocity state.
...
owned/working limits. The state/transport overloads must keep their bit-exact
fingerprints. Zero adjustment must reproduce the exact numeric controls and
diagnostics of the raw-transport path with distinct lineage; nonzero adjustment
must propagate into the next transport. Do not persist an adjusted-source
transport through the existing `SWRM` cycle-state codec until that codec and
cycle-state contract explicitly retain its third lineage.
