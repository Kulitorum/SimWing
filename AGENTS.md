...
- `simwing_scene_fluid_opening_cap`: builds topology-only virtual caps for the
  ...
  once per cell or non-periodic face and retain their clipped polygons. These
  stages do not sample or solve fluid flux.
- `simwing_scene_fluid_cell_volume`: reconstructs bounded deterministic sparse
  ...
  volumes. Opening flux, nonplanar/concave mouths, surface junctions, and
  periodic-boundary ambiguity remain unsupported.
...
There are 139 configured tests on Windows. The Fortran-reference test is
...
| SimWing capped cell volumes/opening kinematics | `simwing_scene_fluid_opening_cap`, `simwing_scene_fluid_opening_quadrature`, `simwing_scene_fluid_opening_patch`, `simwing_scene_fluid_cell_volume` plus the authoritative scene/grid geometry checks when their inputs change; preserve accepted-state/epoch binding, boundary-edge-complete topology-only cap winding, topology-stable one-point triangle kinematics, exact piecewise-linear normal surface sweep and rigid material-plus-cap GCL closure, exact polygon partition with unique cell/non-periodic-face ownership across accepted motion, periodic-boundary ambiguity rejection, no cap-to-Structure/traction path, deterministic signed-tetrahedron clipping and sparse ordering, exact analytic nested/open/large-cavity volumes including full interior cells, local/global closure, bounded work/publication, and explicit rejection of nonplanar/concave openings or non-manifold/inconsistent winding |
