...
- `scene_fluid_mimetic_region_conductance_audit.{h,cpp}` is the graph-
  ...
  `0.0608388978079532 m`. This is an offline Neumann terminal experiment, not
...
- `scene_pressure_cell_mimetic_conductance_phase_refinement_audit.{h,cpp}`
  ...
  24 requested sample records, nested terminal responses, conditional
  statistics, bounds, and exact typed local-cell linear-consistency failures.
  All `8/8` phases solve at `2^3` and `4^3`, with normalized mean/CV
  `0.100660/0.01043` and `0.240930/0.39050`. At `8^3`, `3/8` solve with
  conditional mean/CV `0.420468/0.22404`; the other five retain their exact
