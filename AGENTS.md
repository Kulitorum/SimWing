...
- `simwing_amr_external_flow`: opt-in AMReX 26.06 production-grid evaluation.
  ...
  positive-Y wind-tunnel physical ghosts. The separately enabled
  ...
  positive Y. Its persistent state fills physical and divergence-preserving
  ...
  performs a fresh composite projection every step. It has no viscosity,
  turbulence, or wing boundary yet. All are excluded from ordinary builds.
...
| SimWing AMR external flow | opt-in `simwing_amr_external_flow`, `simwing_amr_external_flow_projection`, and `simwing_amr_external_flow_case`; preserve the pinned AMReX version, default-build independence, two-level non-periodic block layout, face-centred X/Y/Z storage, explicit `-Y` inflow / `+Y` outlet / X-Z far-field ownership, nonuniform divergent wake, exact physical-ghost checks, finite byte accounting, coarse/fine face-flux synchronization before and after projection, homogeneous pressure-correction Neumann ownership at prescribed-normal-velocity boundaries, the `+Y` homogeneous Dirichlet pressure reference, strong composite divergence contraction, exact inlet-normal preservation, nonzero outlet adjustment, owning X-fast coarse diagnostic copies with covered values averaged down, 24,576-cell slice metadata, bounded donor-cell passive-marker transport at CFL below one, divergence-preserving fine/coarse face ghost fill, bounded donor-cell staggered momentum prediction on both levels, physical normal-velocity re-enforcement, fresh per-step composite projection, transactional face-state rollback, deterministic immutable frames, positive-Y motion, and the explicit no-viscosity/no-turbulence/no-wing/no-aerodynamics claim |
