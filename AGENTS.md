...
- `simwing_scene_fluid_opening_cap`: builds topology-only virtual caps for
  ...
  accepted pressure epoch owns this crossing product, ready for the capped
  material-plus-opening face partition without making the cap fabric. A
...
- `simwing_scene_fluid_pressure_face_link`: binds those pressure unknowns
  ...
  centroids. A patch without positive projected centroid separation retains its
  exact count and area as unresolved and publishes no fabricated link.
  Material/open-chain/coplanar overlap and ambiguous inactive faces remain
  explicit and unlinked.
- `simwing_scene_fluid_pressure_operator`: assembles those links into a
  ...
  embedded patches reject instead of becoming implicit sealed walls. Its
...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  projected cell-region centroid separation as explicit unresolved patch count
  and area, publishes no fabricated conductance link, and is rejected by
  pressure-operator assembly. Resolving those conductances, junction face
  areas, and the subsequent worker path remain open.
...
There are 158 configured tests on Windows. The Fortran-reference test is
