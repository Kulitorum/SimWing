...
- `simwing_amr_external_flow`: opt-in AMReX 26.06 production-grid evaluation.
  Its current two-level CPU canonical owns face-centred storage and explicit
  positive-Y wind-tunnel physical ghosts; it does not yet project or transport
  flow and is excluded from ordinary builds. Configure with
  `-DSIMWING_ENABLE_AMREX_BACKEND=ON`; supply an installed dependency through
  `-DSIMWING_AMREX_ROOT=<prefix>` or allow the pinned FetchContent fallback.
...
| SimWing AMR external-flow boundary | opt-in `simwing_amr_external_flow`; preserve the pinned AMReX version, default-build independence, two-level non-periodic block layout, face-centred X/Y/Z storage, explicit `-Y` inflow / `+Y` outlet / X-Z far-field ownership, nonuniform interior wake, exact physical-ghost checks, finite byte accounting, and the explicit no-projection/no-aerodynamics claim |
