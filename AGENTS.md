...
- `simwing_fluid`: Qt-free periodic staggered-grid field operators and
  ...
  solve closes that nonlinear endpoint law against the periodic sharp
  projection with optional separately prescribed jumps. It also owns
...
- `src/fsi/fluid/porous_interface.{h,cpp}` applies a calibrated normal
  ...
  volume flux, and nonnegative dissipation. Its bounded endpoint Picard solver
  repeatedly samples that law and projects the original predicted MAC field
  until both normal-velocity and pressure-jump residuals close. Optional
  ...
  tiles and prescribed sheet-normal motion, but is not moving cut-cell topology
  or a second-order coupled time integrator.
