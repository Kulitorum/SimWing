...
There are 119 configured tests on Windows. The Fortran-reference test is
...
| SimWing authoritative scene/grid geometry | `simwing_scene_fluid_surface`, `simwing_scene_fluid_surface_transfer`, `simwing_scene_surface_geometry`, `simwing_scene_surface_intersection`, `simwing_scene_surface_clipping`, `simwing_scene_surface_ownership`; preserve complete accepted-state fingerprints, canonical bounded broad/narrow phases, exact barycentric clipping and area partition, explicit point/segment contact, duplicate shared-plane masks, unique internal face ownership with authored sides/winding, and periodic-boundary rejection before image ownership exists |
