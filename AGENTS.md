...
- `scene_fluid_mimetic_trace_solve.{h,cpp}` is the audit-only transactional
  ...
  finite, reduces the residual, and rolls back when truncated. Jacobi is not
  yet a practical full-wing preconditioner; stronger local/multilevel
...
- `src/model/nurbs_model.{h,cpp}` can export scene-v2 directly from analytical
  ...
  lie on the positive side of each local cap plane, so never hide this
   ...
   exact rollback when truncated. Exact local wall-trace condensation accepts
   all coarse controls and its global 42,927-row shared system is assembled
   with full wall reconstruction. Stronger preconditioning and production
