...
{
    // LEP embedding patch: children constructed below call MainFrame::self(),
    // which must resolve to this instance when the frame is created directly
    // with a parent instead of through self().
    _self = this;
    // LEP embedding patch: only self-delete when running as a top-level
    // window; the embedded frame is owned by the host widget tree.
...
//    m_pRedoAFoilAct->setShortcut(QKeySequence::Redo); // bug in Qt libs: shortcut is defined twice in translation files
    ...
    m_pZoomLessAct= new QAction(QIcon(":/images/OnZoomOut.png"), tr("Zoom Less"), this);
...
void MainFrame::createToolbars()
{
    ...
    if(m_ptbAFoil->iconSize().height()<minsize)         m_ptbAFoil->setIconSize(QSize(minsize,minsize));
    if(m_ptbMiarex->iconSize().height()<minsize)        m_ptbMiarex->setIconSize(QSize(minsize,minsize));
    if(m_ptbXDirect->iconSize().height()<minsize)       m_ptbXDirect->setIconSize(QSize(minsize,minsize));
    if(m_ptbXInverse->iconSize().height()<minsize)      m_ptbXInverse->setIconSize(QSize(minsize,minsize));

    m_pMiarex->setIconSize(QSize(minsize,minsize));
}
...
{
    m_pEditQSpec = new QAction(tr("Edit Q-spec"), this);
    connect(m_pEditQSpec, SIGNAL(triggered()), m_pXInverse, SLOT(onEditQSpec()));
...
void MainFrame::createXInverseMenus()
{
    ...
    m_pXInverseFoilMenu = menuBar()->addMenu(tr("Foil"));
    {
        ...
        m_pXInverseFoilMenu->addSeparator();
        m_pXInverseFoilMenu->addAction(m_pEditQSpec);
    ...
    m_pInverseContextMenu = new QMenu(tr("Context Menu"),this);
    {
        ...
        m_pInverseContextMenu->addSeparator();
        m_pInverseContextMenu->addAction(m_pEditQSpec);
...
/**
 * LEP embedding patch: imports the wing exported by LEparagliding's
 * Section 36 — every rib airfoil (*.dat) plus a wing-only Plane built from
 * the *.xwimp definition — replacing any previous import of the same name,
 * then switches to the Miarex module with the plane selected.
 */
bool MainFrame::lepImportLepWing(QString const &xflr5DirPath, QString &errorMessage)
{
    QDir xflr5Dir(xflr5DirPath);
    if(!xflr5Dir.exists())
    {
        errorMessage = "Folder not found: " + xflr5DirPath;
        return false;
    }

    // Rib airfoils. Objects2d::insertThisFoil overwrites foils of the same
    // name, which keeps repeated transfers idempotent.
    QStringList const datFiles = xflr5Dir.entryList(QStringList()<<"*.dat", QDir::Files, QDir::Name);
    for(QString const &datName : datFiles)
    {
        QFile datFile(xflr5Dir.filePath(datName));
        if(!datFile.open(QIODevice::ReadOnly)) continue;
        Foil *pFoil = xfl::readFoilFile(datFile);
        if(pFoil) Objects2d::insertThisFoil(pFoil);
    }

    QStringList const wingFiles = xflr5Dir.entryList(QStringList()<<"*.xwimp", QDir::Files, QDir::Name);
    if(wingFiles.isEmpty())
    {
        errorMessage = "No .xwimp wing definition found in " + xflr5DirPath;
        return false;
    }

    Plane *pPlane = new Plane;
    pPlane->setWings(false, false, false);  // a paraglider is a wing-only aircraft
    if(!pPlane->wing()->importDefinition(xflr5Dir.filePath(wingFiles.first()), errorMessage))
    {
        delete pPlane;
        return false;
    }
    pPlane->setName(pPlane->wing()->name());
    pPlane->computePlane();

    Plane *pOldPlane = Objects3d::getPlane(pPlane->name());
    if(pOldPlane)
    {
        // Silent overwrite: the upstream geometry changed, so previous
        // results are stale. Objects3d::setModPlane is not used here
        // because it pops an interactive rename dialog.
        m_pMiarex->m_pCurPlane  = nullptr;
        m_pMiarex->m_pCurWPolar = nullptr;
        m_pMiarex->m_pCurPOpp   = nullptr;
        m_pMiarex->m_pPlaneTreeView->removePlane(pOldPlane);
        Objects3d::deletePlane(pOldPlane);
    }
    Objects3d::addPlane(pPlane);

    m_pMiarex->m_pCurPlane  = pPlane;
    m_pMiarex->m_pCurWPolar = nullptr;
    m_pMiarex->m_pCurPOpp   = nullptr;
    m_pMiarex->setPlane(pPlane->name());
    m_pMiarex->m_pPlaneTreeView->insertPlane(pPlane);
    m_pMiarex->m_pPlaneTreeView->update();
    m_pMiarex->m_pPlaneTreeView->selectPlane(pPlane);
    gl3dMiarexView::s_bResetglMesh = true;
    gl3dMiarexView::s_bResetglLegend = true;

    setSaveState(false);
    onMiarex();
    m_pMiarex->on3DView();  // land on the 3d wing view, whatever was last used
    return true;
}
