...
//    m_pAngleControlModel->setRowCount(17);//temporary
    ...
    if(!m_pPlane->isWing())
    {
        ...
        m_pAngleModel->setData(ind, tr("Wing Tilt (")+DEGCHAR+")");
    ...
    QVBoxLayout *pMethodPageLayout = new QVBoxLayout;
    {
        QVBoxLayout *pMethodLayout = new QVBoxLayout;
        {
            ...
            QVBoxLayout *pViscousLayout = new QVBoxLayout;
            {
                ...
                QLabel *plab11 = new QLabel(tr("Note : the analysis may be of the viscous type only if all the flap controls are inactive"));
                pViscousLayout->addWidget(m_pchViscous);
                pViscousLayout->addWidget(plab11);
            }
        ...
        QGroupBox *pAttitudeGroupBox = new QGroupBox(tr("Flight attitude"));
        {
            ...
            {
                QLabel *plab2 = new QLabel("<p>&beta; =</p>");
                QLabel *plab3 = new QLabel("<p>&phi; =</p>");
                plab2->setAlignment(Qt::AlignVCenter|Qt::AlignRight);
                plab3->setAlignment(Qt::AlignVCenter|Qt::AlignRight);
                QLabel *plab4 = new QLabel("<p>&deg;</p>");
                QLabel *plab5 = new QLabel("<p>&deg;</p>");
                m_pdeBeta  = new DoubleEdit(0.818,2);
                m_pdePhi   = new DoubleEdit(0.414,2);

                pAttitudeLayout->addWidget(plab2,1,1);
                pAttitudeLayout->addWidget(m_pdeBeta,1,2);
                pAttitudeLayout->addWidget(plab4 ,1,3);
                pAttitudeLayout->addWidget(plab3,2,1);
                pAttitudeLayout->addWidget(m_pdePhi,2,2);
                pAttitudeLayout->addWidget(plab5,2,3);
