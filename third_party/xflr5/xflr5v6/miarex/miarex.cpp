...
Miarex::Miarex(QWidget *parent) : QWidget(parent)
{
    ...
    m_StabPlrGraph.at(0)->setXTitle("Real");
    m_StabPlrGraph.at(0)->setYTitle("Imag/2"+PICHAR);
    ...
    m_StabPlrGraph.at(1)->setXTitle("Real");
    m_StabPlrGraph.at(1)->setYTitle("Imag/2"+PICHAR);
...
void Miarex::paintPlaneLegend(QPainter &painter, Plane const*pPlane, WPolar const*pWPolar, QRect const &drawRect, float devicePixelRatio)
{
    ...
    str1 = QString(tr("TipTwist       =")+"%1").arg(pPlane->m_Wing[0].tipTwist(), 10,'f', 3) + DEGCHAR;
    painter.drawText(LeftPos, ZPos+D, str1);
    D+=dheight;
    ...
    str1 = QString(tr("Root-Tip Sweep =")+"%1").arg(pPlane->m_Wing[0].averageSweep(), 10,'f',3) + DEGCHAR;
    painter.drawText(LeftPos, ZPos+D, str1);
    D+=dheight;
...
void Miarex::paintPlaneOppLegend(QPainter &painter, QRect drawRect)
{
    ...
    Result = ALPHACHAR + QString(" = %1").arg(m_pCurPOpp->alpha(), e,'f',f) + DEGCHAR + QString::fromUtf8("  ");
    D+=dheight;
    painter.drawText(RightPos, ZPos+D, dwidth, dheight, Qt::AlignRight | Qt::AlignTop, Result);

    Result = BETACHAR + QString(" = %1").arg(m_pCurPOpp->m_Beta, e,'f',f) + DEGCHAR + QString::fromUtf8("  ");
    D+=dheight;
    painter.drawText(RightPos, ZPos+D, dwidth, dheight, Qt::AlignRight | Qt::AlignTop, Result);
...
void Miarex::setAnalysisParams()
{
    ...
    else if(m_pCurWPolar && m_pCurWPolar->isBetaPolar())
    {
        ...
        m_plabParameterName->setText("<p>&beta;</p>");
    }
    else
    {
        m_plabUnit1->setText("<p>&deg;</p>");
        ...
        m_plabParameterName->setText("<p>&alpha;</p>");
    }
}
...
void Miarex::setupLayout()
{
    QGroupBox *pAnalysisBox = new QGroupBox(tr("Analysis settings"));
    {
        QVBoxLayout *pAnalysisGroupLayout = new QVBoxLayout;
        {
            ...
            {
                m_plabParameterName = new QLabel("<p>&alpha;</p>");
                QLabel *plabAlphaMin   = new QLabel(tr("Start="));
                QLabel *plabAlphaMax   = new QLabel(tr("End="));
                QLabel *plabAlphaDelta = new QLabel("<p>&Delta;=</p>");
...
//                plabAlphaDelta->setFont(QFont("Symbol"));
                ...
                pSequenceGroupLayout->addWidget(m_plabParameterName,1,1,1,3, Qt::AlignCenter);
...
void Miarex::setIconSize(QSize sz)
{
    int iconSize = DisplayOptions::iconSize();
    if(m_ptbX->iconSize().height()<=iconSize)
    {
        m_ptbX->setIconSize(sz);
        m_ptbY->setIconSize(sz);
        m_ptbZ->setIconSize(sz);
        m_ptbIso->setIconSize(sz);
        m_ptbFlip->setIconSize(sz);
    }
}
...
void Miarex::exportToTextStream(WPolar const *pWPolar, QTextStream &out, xfl::enumTextFileType FileType, bool bDataOnly)
{
    ...
    if (FileType==xfl::TXT)
    {
        if(!bDataOnly)
        {
            ...
            {
                strong = QString("Alpha = %1").arg(pWPolar->Alpha()) + DEGCHAR + "\n";
            }
            else strong = "\n";
