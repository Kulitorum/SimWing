...
gl3dMiarexView::gl3dMiarexView(QWidget *parent) : gl3dXflView(parent)
{
    ...
    m_bWakePanels = false; // debug only
...
//        m_matModel.rotate(float(pPOpp->alpha()),0.0,1.0,0.0);
        ...
        // apply sideslip
//        if(fabs(pPOpp->beta())>PRECISION) m_matModel.rotate(float(pPOpp->beta()), 0.0, 0.0, 1.0);
    }
    else
    {
//        if(pWPolar && fabs(pWPolar->Beta())>0.001) m_matModel.rotate(float(pWPolar->Beta()), 0.0, 0.0, 1.0);
    }
    ...
    if(m_bVLMPanels)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if(m_bWakePanels)
                {
                    if(bBackground) paintMesh(m_vboWake);
                    paintSegments(m_vboWakeEdges, Qt::magenta, 1);
                }
            }
        }
    }
    ...
    for(int i=0; i<PanelAnalysis::s_DebugPts.size(); i++)
        paintSphere(PanelAnalysis::s_DebugPts.at(i), 0.0075/m_glScalef, Qt::darkCyan);
    for(int i=0; i<PanelAnalysis::s_DebugVecs.size(); i++)
    {
        if(i<PanelAnalysis::s_DebugPts.size())
            paintThinArrow(PanelAnalysis::s_DebugPts.at(i), PanelAnalysis::s_DebugVecs.at(i), Qt::darkRed, 2, Line::SOLID, m_matModel);
    }
#endif
                ...
                if(bFound)
                {
                    ...
                    if(GL3DScales::s_pos==1 && qAbs(GL3DScales::s_XOffset)<0.001 && qAbs(GL3DScales::s_ZOffset)<0.001)
                    {
                        ...
                        if(panel.m_Pos==xfl::BOTSURFACE)
                        {
                            ...
                            VAT = panel_i.TA() - panel_i.LA();
                            VAT.normalize();
                            VA = VA+VAT;
                            VA.normalize();//VA is parallel to the bisector angle

                            VBT = panel_i.TB() - panel_i.LB();
                            VBT.normalize();
                            VB = VB+VBT;
...
void gl3dMiarexView::glMakePanels(QOpenGLBuffer &vboPanels, QVector<Panel> const &panels, const PlaneOpp *pPOpp)
{
    float color(0);
    float range(0);
    ...
    for (int p=0; p<nPanels; p++)
    {
        TA.copy(panels.at(p).TA());
        TB.copy(panels.at(p).TB());
        LA.copy(panels.at(p).LA());
        LB.copy(panels.at(p).LB());
...
//                    glMakeWingGeometry(iw, pCurPlane->wing(iw), nullptr);
    ...
    if(s_bResetglMesh)
    {
        if(!pCurWPolar || pCurWPolar->isLLTMethod())
        {
            ...
            if(pCurPlane->body())
            {
                ...
                glMakePanels(m_vboEditBodyMesh, panels, nullptr);
                glMakePanelEdges(m_vboEditBodyMeshEdges, panels);
            }
        }
        else
        {
            glMakePanels(m_vboMesh, theTask.m_Panel, nullptr);
            if(m_bWakePanels)
                glMakePanels(m_vboWake, theTask.m_WakePanel, nullptr);
        }

        glMakePanelEdges(m_vboPanelEdges, theTask.m_Panel);
        if(m_bWakePanels)
            glMakePanelEdges(m_vboWakeEdges, theTask.m_WakePanel);
    ...
    if(s_bResetglPanelCp || s_bResetglOpp)
    {
        if(pCurWPolar && pCurWPolar->analysisMethod()!=xfl::LLTMETHOD)
            glMakePanels(m_vboPanelCp, theTask.m_Panel, pCurPOpp);
