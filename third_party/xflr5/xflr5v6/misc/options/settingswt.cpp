...
Settings::Settings(QWidget *pParent) : QWidget(pParent)
{
    ...
    connect(m_pieIconSize,           SIGNAL(valueChanged()),             SLOT(onIconSize()));
...
void Settings::setupLayout()
{
    QGroupBox *pWidgetStyleBox = new QGroupBox(tr("Widget Style"));
    {
        QVBoxLayout *pWidgetStyleLayout = new QVBoxLayout;
        {
            ...
            pWidgetStyleLayout->addLayout(pIconLayout);
        }
...
void Settings::onStyleChanged()
{
    ...
    // LEP embedding patch: when MainFrame is embedded in a host application,
    // leave the host's application-wide QStyle untouched.
    if(!MainFrame::self() || MainFrame::self()->isWindow())
...
void Settings::saveSettings(QSettings &settings)
{
    settings.beginGroup("global_settings");
    {
        ...
        settings.setValue("IconSize", DisplayOptions::iconSize());
...
void Settings::loadSettings(QSettings &settings)
{
    settings.beginGroup("global_settings");
    {
        ...
        DisplayOptions::setIconSize(settings.value("IconSize", DisplayOptions::iconSize()).toInt());
...
void Settings::onStyleSheet(bool bSheet)
{
    ...
    // LEP embedding patch: when MainFrame is embedded in a host application,
    // scope the stylesheet to the MainFrame subtree instead of the whole app.
    QWidget *pEmbeddedFrame = nullptr;
    if(MainFrame::self() && !MainFrame::self()->isWindow())
        pEmbeddedFrame = MainFrame::self();
    QFile stylefile;
    if(bSheet)
    {
        ...
        if (stylefile.open(QIODevice::ReadOnly | QIODevice::Text))
        {
            ...
            if(pEmbeddedFrame) pEmbeddedFrame->setStyleSheet(qsStylesheet);
            else               qApp->setStyleSheet(qsStylesheet);
            stylefile.close();
            QApplication::restoreOverrideCursor();
        }
    }
    ...
    {
        if(pEmbeddedFrame) pEmbeddedFrame->setStyleSheet(QString());
