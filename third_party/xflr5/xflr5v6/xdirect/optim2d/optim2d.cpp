...
void Optim2d::showEvent(QShowEvent *pEvent)
{
...
void Optim2d::hideEvent(QHideEvent *pEvent)
{
...
Foil *Optim2d::onStoreBestFoil()
{
    ...
    QString strange = QString::asprintf("Optimized at aoa=%g", s_Alpha) + DEGCHAR;
    if(s_bTEFlap) strange += QString::asprintf("  Flap angle=%5.3g", m_FlapAngle) + DEGCHAR;
    strange +=" for: ";
...
void Optim2d::onIterEvent(OptimEvent *result)
{
    ...
    if(m_pPSOTask)
    {
        ...
        if(s_bTEFlap) strange += QString::asprintf("  Flap angle=%5.2g", particle.position().last()) + DEGCHAR+"  ";
        for(int iobj=0; iobj<m_pPSOTask->nObjectives(); iobj++)
        {
            OptObjective const &obj = m_pPSOTask->objective(iobj);
            strange += " " + obj.m_Name;
