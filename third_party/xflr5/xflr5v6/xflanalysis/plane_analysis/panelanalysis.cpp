...
//    for(int i=0; i<m_Panel.size(); i++) displayDouble(m_uRHS[i], m_wRHS[i]);
...
//for(int i=0; i<m_Panel.size(); i++) displayDouble(m_uWake[i], m_wWake[i]);
...
//display_vec(m_aij, 2*m_Panel.size());
//for(int i=0; i<m_Panel.size(); i++) displayDouble(m_uRHS[i], m_wRHS[i]);
...
//for(int i=0; i<m_Panel.size(); i++) displayDouble(m_uRHS[i], m_wRHS[i]);
    ...
    if (s_bCancel) return true;
//for(int i=0; i<m_Panel.size(); i++) displayDouble(m_Sigma[i], m_Mu[i]);
    ...
    if (s_bCancel) return true;
//for(int i=0; i<m_Panel.size(); i++)    displayDouble(m_Cp[i]);
...
void PanelAnalysis::buildInfluenceMatrix()
{
    ...
    for(p=0; p<m_Panel.size(); p++)
    {
        ...
        for(pp=0; pp<m_Panel.size(); pp++)
        {
            ...
            getDoubletInfluence(C, m_Panel.at(pp), V, phi, true);
...
*/
void PanelAnalysis::createRHS(double *RHS, Vector3d const &VInf, double *VField)
{
    ...
    for (int p=0; p<MatSize; p++)
    {
        ...
        Panel const &destpanel = m_Panel.at(p);
        ...
        //        if(!m_b3DSymetric || m_Panel[p].m_bIsLeftPanel)
        //        {
        if(!m_pWPolar->bDirichlet() || destpanel.m_Pos==xfl::MIDSURFACE)
        {
            // first term of RHS is -V.n
            RHS[m] = - destpanel.Normal.dot(VPanel);
            //                if(RHS[m]<PRECISION) RHS[m] = 0.0;
        }
        else if(m_pWPolar->bDirichlet()) RHS[m] = 0.0;
        if(destpanel.m_Pos!=xfl::MIDSURFACE) C = destpanel.CollPt;
        else                                 C = destpanel.CtrlPt;

        for (int pp=0; pp<MatSize; pp++)
        {
            // Only consider the panels positioned on thick surfaces,
            // since the source strength is zero on thin surfaces

            Panel const &srcpanel = m_Panel.at(pp);

            if(srcpanel.m_Pos!=xfl::MIDSURFACE)
            {
                // Define the source strength on panel pp
                sigmapp = -1.0/4.0/PI * srcpanel.Normal.dot(VPanel);

                // Add to RHS the source influence of panel pp on panel p
                getSourceInfluence(C, srcpanel, V, phi);
                ...
                if(!m_pWPolar->bDirichlet() || destpanel.m_Pos==xfl::MIDSURFACE)
                {
                    ...
                    RHS[m] -= V.dot(destpanel.Normal) * sigmapp;
                }
...
//            qDebug(" %4d  %13f  %13f  %13f  ", pp, RHS[m], phi, sigmapp);
        }
        m++;
...
void PanelAnalysis::createWakeContribution()
{
    ...
    for(p=0; p<Size; p++)
    {
        ...
        for (kw=0; kw<m_NWakeColumn; kw++)
        {
            ...
            {
                Panel const &wakepanel = m_WakePanel.at(pw);
                getDoubletInfluence(C, wakepanel, V, phi, true);

                PHC[kw] += phi;
                VHC[kw] += V;
//qDebug(" %5d  %13f  %13f  %13f  %13f", pw, phi, V.x, V.y, V.z);
                pw++;
            }
//            qDebug(" %5d   %5d  %13f", p, kw, PHC[kw]);
        }
        ...
        for(pp=0; pp<Size; pp++) //for each matrix column
        {
            ...
            Panel const &TEPanel = m_Panel.at(pp);

            // Is the panel pp shedding a wake?
            if(TEPanel.isTrailing())
            {
                // If so, we need to add the contributions of the wake column
                // shedded by this panel to the RHS and to the Matrix
                // Get trailing point where the jump in potential is evaluated v6.02
                TrPt = (TEPanel.TA() + TEPanel.TB())/2.0;

                if(TEPanel.m_Pos==xfl::MIDSURFACE)
                {
                    ...
                    if(!m_pWPolar->bDirichlet() || destpanel.m_Pos==xfl::MIDSURFACE)
                    {
                        ...
                        m_aijWake[m*Size+mm] += VHC[TEPanel.m_iWakeColumn].dot(destpanel.Normal);
                    ...
                    else if(m_pWPolar->bDirichlet())
                    {
                        ...
                        m_aijWake[m*Size+mm] += PHC[TEPanel.m_iWakeColumn];
                        //we do not add the term Phi_inf_KWPUM - Phi_inf_KWPLM (eq. 44) since it is 0, thin edge
                    }
                }
                else if(TEPanel.m_Pos==xfl::BOTSURFACE)
                {
                    //the panel shedding a wake is on the bottom side, substract
                    if(!m_pWPolar->bDirichlet() || destpanel.m_Pos==xfl::MIDSURFACE)
                    {
                        //use Neumann B.C.
                        m_aijWake[m*Size+mm] -= VHC[TEPanel.m_iWakeColumn].dot(destpanel.Normal);
                        //corrected in v6.02;
                        m_uWake[m] -= TrPt.x  * VHC[TEPanel.m_iWakeColumn].dot(destpanel.Normal);
                        m_wWake[m] -= TrPt.z  * VHC[TEPanel.m_iWakeColumn].dot(destpanel.Normal);
                    }
                    else if(m_pWPolar->bDirichlet())
                    {
                        m_aijWake[m*Size+mm] -= PHC[TEPanel.m_iWakeColumn];
                        m_uWake[m] +=  TrPt.x * PHC[TEPanel.m_iWakeColumn];
                        m_wWake[m] +=  TrPt.z * PHC[TEPanel.m_iWakeColumn];
                    }
                }
                else if(TEPanel.m_Pos==xfl::TOPSURFACE)
                {
                    //the panel shedding a wake is on the top side, add
                    if(!m_pWPolar->bDirichlet() || destpanel.m_Pos==xfl::MIDSURFACE)
                    {
                        //use Neumann B.C.
                        m_aijWake[m*Size+mm] += VHC[TEPanel.m_iWakeColumn].dot(destpanel.Normal);
...
//qDebug(" %5d  %5d  %13f  %13f  ", p, mm, m_uWake[m], m_uWake[m]);
            }
            mm++;
        }
        m++;
...
void PanelAnalysis::createWakeContribution(double *pWakeContrib, const Vector3d &WindDirection)
{
    ...
    for(int p=0; p<MatSize; p++)
    {
        ...
        for (int kw=0; kw<m_NWakeColumn; kw++)
        {
            ...
            {
                getDoubletInfluence(C, m_WakePanel.at(pw), V, phi, true);
...
void PanelAnalysis::computeFarField(double QInf, double Alpha0, double AlphaDelta, int nval)
{
    ...
    for(int iw=0; iw<MAXWINGS; iw++)
    {
        if(m_pWingList[iw]) ThinSize += m_pWingList[iw]->m_nPanels;
    }
...
//                memcpy(m_Vd  + (q*MAXWINGS+iw)*m_NSpanStations, m_pWingList[iw]->m_Vd,  uint(m_pWingList[iw]->m_NStation)*sizeof(Vector3d));
                ...
                m_Progress += 10.0 * double(m_pWingList[iw]->m_nPanels)/double(ThinSize) *double(MatSize)/400.;
                if(s_bCancel)return;
            }
        }
    }
...
void PanelAnalysis::computeAeroCoefs(double V0, double VDelta, int nrhs)
{
    ...
    else
    {
        for (int q=0; q<nrhs; q++)
        {
            ...
            if(m_3DQInf[q]>0.0)
            {
                if(!m_pWPolar->bTilted()) str = QString("      Computing Plane for " + ALPHACHAR + "=%1").arg(V0+q*VDelta,7,'f',2);
                else                      str = QString("      Computing Plane for " + ALPHACHAR + "=%1").arg(m_OpAlpha,7,'f',2);
                str += DEGCHAR+("\n");
                traceLog(str);
...
void PanelAnalysis::getDoubletInfluence(Vector3d const &C, Panel const &panel, Vector3d &V, double &phi, bool bAll) const
{
    if(panel.m_Pos!=xfl::MIDSURFACE || panel.m_bIsWakePanel)
        phi = panel.doubletNASA4023(C, V);
    else
    {
    ...
    if(m_pWPolar->bGround())
    {
        ...
        if(panel.m_Pos!=xfl::MIDSURFACE || panel.m_bIsWakePanel)    phiG = panel.doubletNASA4023(CG, VG);
        else
        {
...
void PanelAnalysis::getSourceInfluence(Vector3d const &C, Panel const&panel, Vector3d &V, double &phi) const
{
    ...
    if(m_pWPolar->bGround())
    {
        ...
        Vector3d VG;
        Vector3d CG(C.x, C.y, -C.z-2.0*m_pWPolar->m_Height);
        phiG = panel.sourceNASA4023(CG, VG);
        V.x += VG.x;
        V.y += VG.y;
        V.z -= VG.z;
        phi += phiG;
...
void PanelAnalysis::getSpeedVector(Vector3d const &C, double const *Mu, double const *Sigma, Vector3d &VT, bool bAll) const
{
    ...
    for (int pp=0; pp<MatSize;pp++)
    {
        ...
        Panel const &panel= m_Panel.at(pp);

        if(panel.m_Pos!=xfl::MIDSURFACE) //otherwise Sigma[pp] =0.0, so contribution is zero also
        {
            getSourceInfluence(C, panel, V, phi);
            VT += V * Sigma[pp];
        }
        getDoubletInfluence(C, panel, V, phi, bAll);

        VT += V * Mu[pp];

        // Is the panel pp shedding a wake ?
        if(panel.m_bIsTrailing && panel.m_Pos!=xfl::MIDSURFACE)
        {
            //If so, we need to add the contribution of the wake column shedded by this panel
            if(panel.m_Pos==xfl::BOTSURFACE) sign=-1.0; else sign=1.0;
            int pw = panel.m_iWake;
            for(int lw=0; lw<m_pWPolar->m_NXWakePanels; lw++)
            {
                getDoubletInfluence(C, m_WakePanel.at(pw+lw), V, phi, bAll);
                VT += V * Mu[pp]*sign;
            }
        }
// VT.listCoords(pp);
...
bool PanelAnalysis::QInfLoop()
{
    ...
    if(m_pWPolar->bTilted())
    {
        ...
        Vector3d O;
        rotateMeshY(m_pWPolar->m_AlphaSpec, O);
...
bool PanelAnalysis::unitLoop()
{
    ...
    const int MatSize = m_Panel.size();
    ...
    for (int n=0; n<m_nRHS; n++)
    {
        ...
        // Rotate the wing panels and translate the wake to the new T.E. position
        if(fabs(m_OpAlpha)>PRECISION)
            rotateMeshY(m_OpAlpha, O);

        //        if(m_pWPolar->polarType()==XFLR5::BETAPOLAR)
        if(fabs(m_OpBeta)>PRECISION)
        {
            rotateMeshZ(m_OpBeta, O);
        }
...
bool PanelAnalysis::computeTrimmedConditions()
{
    ...
    strong += ALPHACHAR + QString("=%1").arg(m_AlphaEq,0,'f',5) + DEGCHAR +"\n";
    traceLog(strong);
...
{
    if(m_pWPolar && (m_pWPolar->polarType()==xfl::STABILITYPOLAR || m_pWPolar->bTilted()))
    {
 ...
 */
void PanelAnalysis::rotateMeshY(double Alpha, Vector3d const &P)
{
    ...
    for (int p=0; p<m_Panel.size(); p++)
    {
        int iLA = m_Panel.at(p).m_iLA;
        int iLB = m_Panel.at(p).m_iLB;
        int iTA = m_Panel.at(p).m_iTA;
        int iTB = m_Panel.at(p).m_iTB;
...
void PanelAnalysis::rotateMeshZ(double Beta, Vector3d const &P)
{
    ...
    for (int p=0; p<m_Panel.size(); p++)
    {
        ...
        if      (m_Panel.at(p).m_Pos>=xfl::MIDSURFACE) m_Panel[p].setPanelFrame(m_Node.at(iLA), m_Node.at(iLB), m_Node.at(iTA), m_Node.at(iTB));
        else if (m_Panel.at(p).m_Pos==xfl::BOTSURFACE) m_Panel[p].setPanelFrame(m_Node.at(iLB), m_Node.at(iLA), m_Node.at(iTB), m_Node.at(iTA));
    }
}
...
void PanelAnalysis::panelTrefftz(Wing *pWing, double QInf, double Alpha, double const*Mu, double const*Sigma, int pos,
                                 ...
                                 WPolar const*pWPolar, QVector<Panel> const&WakePanel) const
{
    int nw(0);
    int pp(0);
    ...
    for (int j=0; j<NSurfaces; j++)
    {
        ...
        for (int k=0; k<pSurf->nYPanels(); k++)
        {
            ...
            {
                pWing->m_StripArea[m] += m_Panel.at(pWing->firstPanelIndex()+pp).area();
                pp++;
            }
            pWing->m_StripArea[m] /= double(coef);

            Panel const &panel = m_Panel.at(pWing->firstPanelIndex()+p);
            ...
            if(!pWPolar->bThinSurfaces())
            {
                ...
                C = (WakePanel.at(nw).TA() + WakePanel.at(nw).TB())/2.0;
                getSpeedVector(C, Mu, Sigma, Wg, false);
...
bool PanelAnalysis::createWakeElems()
{
    m_WakePanel.clear();
    m_WakeNode.clear();
...
//    s_DebugPts.clear();
    ...
    int NXWakePanels       = m_pWPolar->m_NXWakePanels;
    double WakePanelFactor = m_pWPolar->m_WakePanelFactor;
    double TotalWakeLength = m_pWPolar->m_TotalWakeLength;

    Q_ASSERT(NXWakePanels==1);

    int m=0;
    for(int p=0; p<m_Panel.size(); p++)
    {
        Panel &apanel = m_Panel[p];

        if(!apanel.isTrailing())
            continue;

        apanel.m_iWake = m_WakePanel.size();
        apanel.m_iWakeColumn = m;

        if(apanel.isBotSurface()) continue; // will use the top surface panel as the shedder at the next iteration

        m_WakePanel.push_back(Panel());
        Panel &wakepanel = m_WakePanel.last();

        LA = apanel.TA();
        LB = apanel.TB();

        double dxA = TotalWakeLength*m_pPlane->mac() - apanel.TA().x;
        double dxB = TotalWakeLength*m_pPlane->mac() - apanel.TB().x;
...
/*        wakepanel.m_Vtx[0] = m_WakeNode.at(wakepanel.m_iLA);
        wakepanel.m_Vtx[1] = m_WakeNode.at(wakepanel.m_iLB);
        wakepanel.m_Vtx[2] = m_WakeNode.at(wakepanel.m_iTA);
        wakepanel.m_Vtx[3] = m_WakeNode.at(wakepanel.m_iTB);*/


        bool bDebug = false;
        if(bDebug)
        {
            s_DebugPts.append(wakepanel.m_Vtx[0]);
            s_DebugPts.append(wakepanel.m_Vtx[1]);
            s_DebugPts.append(wakepanel.m_Vtx[2]);
            s_DebugPts.append(wakepanel.m_Vtx[3]);
        }
        ...
        wakepanel.m_iPD = -1; // no panel downstream
        wakepanel.m_iPU = -1; // no panel upstream
