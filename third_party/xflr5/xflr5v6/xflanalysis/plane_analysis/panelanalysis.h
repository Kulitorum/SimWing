...
class PanelAnalysis : QObject
{
    ...
    public:
        ...
        void createRHS(double *RHS, const Vector3d &VInf, double *VField=nullptr);
        void createUnitRHS();
        void createWakeContribution();
        void createWakeContribution(double *pWakeContrib, Vector3d const &WindDirection);
        void getDoubletInfluence(Vector3d const &C, const Panel &panel, Vector3d &V, double &phi, bool bAll) const;
        ...
        void panelTrefftz(Wing *pWing, double QInf, double Alpha, const double *Mu, const double *Sigma, int pos, Vector3d &Force, double &WingIDrag,
                          const WPolar *pWPolar, const QVector<Panel> &WakePanel) const;
        ...
        void releaseArrays();
        void rotateMeshY(double Alpha, Vector3d const &P);
        void rotateMeshZ(double Beta, Vector3d const &P);

        void traceLog(QString str);

        void setControlPositions(double t, int &NCtrls, QString &out, bool bBCOnly);

        bool createWakeElems();
