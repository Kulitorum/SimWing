...
bool PlaneTask::initializePanels()
{
    ...
    // the currently allocated memory size for the influence matrix.
...
int PlaneTask::createSurfaceElements(Plane const*pPlane, WPolar const*pWPolar, Surface *pSurface)
{
    ...
    if (bThickSurfaces && pWPolar && pSurface->isTipLeft())
    {
        ...
        for (int l=0; l<pSurface->nXPanels(); l++)
        {
            ...
            panel.m_Vtx[0] = m_Node.at(panel.m_iLA);
            panel.m_Vtx[1] = m_Node.at(panel.m_iLB);
            panel.m_Vtx[2] = m_Node.at(panel.m_iTA);
            panel.m_Vtx[3] = m_Node.at(panel.m_iTB);

            panel.m_Pos = xfl::SIDESURFACE;
            panel.m_iIndex = matsize;
    ...
    for (int k=0; k<pSurface->nYPanels(); k++)
    {
        ...
        for (int l=0; l<pSurface->nXPanels(); l++)
        {
            ...
            if      (side==xfl::MIDSURFACE)
                ...
                panel.setPanelFrame(pSurface->LB, pSurface->LA, pSurface->TB, pSurface->TA);
...
int PlaneTask::isWakeNode(Vector3d &Pt) const
{
...
int PlaneTask::isNode(Vector3d &Pt) const
{
...
void PlaneTask::PanelAnalyze()
{
    ...
    m_WakeNode  = m_pthePanelAnalysis->m_WakeNode;
    m_WakePanel = m_pthePanelAnalysis->m_WakePanel;
