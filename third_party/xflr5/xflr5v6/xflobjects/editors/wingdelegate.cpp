...
void WingDelegate::setEditorData(QWidget *pEditor, const QModelIndex &index) const
{
    if(index.column()!=5 && index.column()!=7 && index.column()!=9)
    {
...
void WingDelegate::setModelData(QWidget *pEditor, QAbstractItemModel *model, const QModelIndex &index) const
{
    if(index.column()!=5 && index.column()!=7 && index.column()!=9)
    {
        DoubleEdit *pDE = static_cast<DoubleEdit*>(pEditor);
...
void WingDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if(index.column()!=5 && index.column()!=7 && index.column()!=9)
    {
        ...
        strong = QString("%1").arg(pModel->data(index, Qt::DisplayRole).toDouble(),0,'f', m_Precision.at(index.column()));
    }
    else
    {
...
void WingDelegate::updateEditorGeometry(QWidget *pEditor, const QStyleOptionViewItem &option, const QModelIndex &/* index */) const
{
