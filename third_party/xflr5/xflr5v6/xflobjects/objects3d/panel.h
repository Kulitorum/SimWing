...
class Panel
{
    ...
    public:
        ...
        double doubletNASA4023(Vector3d const &C, Vector3d &VTest) const;
        double sourceNASA4023(Vector3d const &C, Vector3d &VTest) const;
        ...
        bool isWakePanel()  const {return m_bIsWakePanel;}
        bool isInSymPlane() const {return m_bIsInSymPlane;}


        Vector3d const &LA() const {return m_Vtx[0];}
        Vector3d const &LB() const {return m_Vtx[1];}
        Vector3d const &TA() const {return m_Vtx[2];}
        Vector3d const &TB() const {return m_Vtx[3];}
    ...
    protected:
        ...
        int m_iIndex;            /**< panel identification number ; used when the panel array is re-arranged in non sequential order to reduce the matrix size in symetrical calculations */
        ...
        Vector3d m_Vtx[4];       /**< corner coordinates in global frame of reference, in the order LA, LB, TA, TB */
