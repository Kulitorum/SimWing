...
#include <xinverse/qspeceditdlg.h>
...
bool XInverse::execQDES()
{
    ...
    {
        // || !pXFoil->liqset*;
        outputMsg(tr("Must mark off target segment first")+"\n");
        return false;
    }
    outputMsg(tr("executing...")+"\n");
...
void XInverse::loadSettings(QSettings &settings)
{
    settings.beginGroup("XInverse");
    {
        ...
        QSpecEditDlg::s_Geometry = settings.value("QSpecGeom", QByteArray()).toByteArray();
    }
    settings.endGroup();
...
void XInverse::saveSettings(QSettings &settings)
{
    settings.beginGroup("XInverse");
    {
        ...
        settings.setValue("QSpecGeom", QSpecEditDlg::s_Geometry);
    }
    settings.endGroup();
                    ...
                    if (bCtrl)
                    {
                        if(iCtrlPt>=0)
                        {
                            if (m_Spline.m_iSelect>=0)
                            {
                                ...
                                {
                                    outputMsg(tr("The minimum number of control points has been reached for this spline degree"));
                                    outputMsg("\n\n");
    ...
    else if(m_bGetPos && m_rCltRect.contains(point))
    {
        ...
        if(m_nPos == 2)
        {
            ...
            else if(m_bSpline)
            {
                ...
                outputMsg(tr("Drag the points to modify the spline, Apply, and Execute to generate the new geometry")+"\n");
            }
            else if(m_bMark)
            {
...
//            int isp = m_pXFoil->nsp - i + 1;
    ...
    outputMsg(tr("Spline is applied")+"\n");
    updateView();
}


void XInverse::onEditQSpec()
{
    if(!m_pMCurve) return;

    QSpecEditDlg dlg;
    dlg.initDialog(m_pMCurve->m_x, m_pMCurve->m_y);
    if(dlg.exec()!=QDialog::Accepted) return;

    dlg.readData(m_pMCurve->m_x, m_pMCurve->m_y);

    for (int i=1; i<m_pMCurve->size()-1; i++) m_pReflectedCurve->m_y[i] = -m_pMCurve->m_y[i];
...
void XInverse::onMarkSegment()
{
    ...
    {
        outputMsg(tr("Mark target segment for modification")+"\n");
    }
...
void XInverse::onNewSpline()
{
    ...
    if((m_bFullInverse && m_ppbNewSpline->isChecked()) || (!m_bFullInverse && m_ppbMNewSpline->isChecked()))
    {
        ...
        outputMsg(tr("Mark spline endpoints")+"\n");
...
void XInverse::onSpecal()
{
    ...
    {
        m_plabSpecif->setText("<p>&alpha;=</p>");
...
void XInverse::onSmooth()
{
    ...
    {
        outputMsg(tr("Mark target segment for smoothing, or type 'Return' to smooth the entire distribution, then Execute"));
        outputMsg("\n\n");
...
void XInverse::setFoil()
{
    ...
    else
    {
        ...
        createQCurve();
        createMCurve();
//        strong = QString::asprintf("Algam=%.3f   Cl=%.3f\n", m_pXFoil->algam*180.0/PI, m_pXFoil->clgam);
//        outputMsg(strong);
    ...
    if(m_pXFoil->lvisc)
    {
        ...
        double x(0), y(0);
        double dsp(0), dqv(0), sp1(0), sp2(0), qv1(0), qv2(0);
...
bool XInverse::initialize()
{
    QString strange, strFoilName;
    ...
    if (XDirect::curFoil())
    {
        ...
        if(!m_pXFoil->lqspec)
        {
            if(m_bFullInverse) m_pXFoil->InitMDES();
            else               m_pXFoil->InitQDES();
        }
    }
    else
    {
...
/*        if(Objects2d::foilCount())
    ...
    if(!m_bFullInverse)
    {
        strange = QString::asprintf("Alpha=%.3f  Cl=%.3f\n\n", m_pXFoil->alfa*180.0/PI, m_pXFoil->cl);
        outputMsg("Last XFoil run:\n"+strange);
    }
...
void XInverse::setupLayout()
{
    QGroupBox *pSpecBox = new QGroupBox(tr("Specification"));
    {
        ...
        {
            m_prbSpecAlpha = new QRadioButton(ALPHACHAR);
            m_prbSpecCl    = new QRadioButton(tr("Cl"));
            m_plabSpecif   = new QLabel("<p>&alpha;=</p>");
            ...
            pSpecLayout->addWidget(m_prbSpecAlpha,1,1, Qt::AlignRight);
            pSpecLayout->addWidget(m_prbSpecCl,   1,2);
            pSpecLayout->addWidget(m_plabSpecif,  2,1, Qt::AlignRight);
            pSpecLayout->addWidget(m_pdeSpec,     2,2);
            pSpecLayout->setColumnStretch(1,1);
            pSpecLayout->setColumnStretch(3,1);
    ...
    QGroupBox *pSmoothBox = new QGroupBox(tr("Smoothing"));
    {
        QGridLayout *pSmoothLayout = new QGridLayout;
        {
            ...
            QLabel *plab0 = new QLabel(tr("Filter parameter"));
            m_pdeFilterParam = new DoubleEdit(0.1);
            m_pdeFilterParam->setToolTip(strong);
            pSmoothLayout->addWidget(m_ppbSmooth,      1,1);
            pSmoothLayout->addWidget(m_ppbFilter,      1,2);
            pSmoothLayout->addWidget(plab0,            2,1, Qt::AlignRight);
    ...
    QGroupBox *pConstraintsBox = new QGroupBox(tr("Constraints"));
    {
        ...
        {
            QLabel *plab1 = new QLabel(tr("T.E. Angle"));
            QLabel *plab2 = new QLabel(tr("T.E. Gap dx/c"));
            QLabel *plab3 = new QLabel(tr("T.E. Gap dy/c"));

            m_pdeTAngle = new DoubleEdit(0.0,3);
            m_pdeTGapx  = new DoubleEdit(0.0,3);
            m_pdeTGapy  = new DoubleEdit(0.0,3);
            QString strong ="XFoil doc: \n"
            ...
            pTELayout->addWidget(plab1,       1,1);
            pTELayout->addWidget(plab2,       2,1);
            pTELayout->addWidget(plab3,       3,1);
            pTELayout->addWidget(m_pdeTAngle, 1, 2, Qt::AlignRight);
            pTELayout->addWidget(m_pdeTGapx,  2, 2, Qt::AlignRight);
            pTELayout->addWidget(m_pdeTGapy,  3, 2, Qt::AlignRight);
            pTELayout->setColumnStretch(1,1);
            pTELayout->setColumnStretch(3,1);
...
void XInverse::smooth(int Pos1, int Pos2)
{
    ...
    {
        //smooth it all
        outputMsg(tr("Smoothing the entire distribution.\n")+"\n");
        Pos1 = 1;
        Pos2 = m_pXFoil->nsp;
    }
    else
        outputMsg(tr("Smoothing the selected portion.\n")+"\n");
...
void XInverse::outputMsg(const QString &msg)
{
